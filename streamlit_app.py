...
st.markdown('<div class="main-header">🪑 Seating Arrangement System</div>', unsafe_allow_html=True)
...
# Main content area
tab1, tab2, tab3 = st.tabs(["📤 Upload & Run", "📊 View Results", "ℹ️ About"])
        ...
        if st.button("🚀 Generate Seating Arrangement", type="primary", use_container_width=True):
            with st.spinner("Processing seating arrangement..."):
                try:
                    ...
                    # Show errors if any
                    if system.errors:
                        st.warning("⚠️ Some errors were detected. Check the errors section below.")
...
with tab2:
    ...
    if st.session_state.get('generated', False):
        ...
        st.info("ℹ️ Please generate seating arrangement first in the 'Upload & Run' tab")
